...
test('creates and validates business hours with generated connectors', async ({ page }) => {
  ...
  await page.getByLabel('Title').fill('Weekend Hours')
  await page.getByLabel('Description').fill('Weekend coverage')
  await page.getByLabel('Type').selectOption('businessHours')
  await page.getByRole('button', { name: 'Create Node' }).click()
  await expect(page.locator('.flow-node')).toHaveCount(10)
  await expect(page.getByText('Weekend Hours', { exact: true }).last()).toBeVisible()
