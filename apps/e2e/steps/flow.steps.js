...
const imageFixture = fileURLToPath(new URL('../fixtures/upload.png', import.meta.url))
const invalidFixture = fileURLToPath(new URL('../fixtures/not-an-image.txt', import.meta.url))

/** @typedef {import('#support/world').FlowWorld} FlowWorld */

/**
 * @param {import('@cucumber/cucumber').IWorld & { flow?: import('#pages/FlowPage').FlowPage | null }} world
 */
function flow(world) {
...
When(
  ...
  step(async function () {
    await flow(this).uploadImage(imageFixture)
  }),
)
...
When(
  ...
  step(async function () {
    await flow(this).uploadImage(invalidFixture)
  }),
)
